...
def convert_tsv_to_markdown(tsv_path: Path, output_path: Path):
    ...
    md_content.append("  const searchInput = document.querySelector('.md-search__input');\n")
    md_content.append("  if(searchInput){\n")
    md_content.append("    searchInput.placeholder = 'Search projects (e.g., python, machine learning, biology)...';\n")
    md_content.append("    searchInput.addEventListener('input', function(e){\n")
    md_content.append("      searchProjects(e.target.value);\n")
    md_content.append("      // Prevent default MkDocs search\n")
    md_content.append("      e.stopPropagation();\n")
    md_content.append("    });\n")
    md_content.append("    // Clear default search behavior\n")
    md_content.append("    searchInput.addEventListener('focus', function(){\n")
